...
// Note: Futures (GC, SI, etc.) and direct indices (SPX, DJI) require paid plans.
...
export const indicatorsService = {
  ...
  async getAllQuotes(): Promise<IndicatorQuote[]> {
    ...
    // Each symbol in a batch = 1 credit. Batch of 4 = 4 credits.
    // With 8.5s delay, we get ~7 batches/min = ~28 credits/min (under limit with margin)
    const BATCH_SIZE = 4;
